...
var StreamChannels = function(apiClient, options) {
  ...
  options.enableFollowsEvents = typeof options.enableFollowsEvents === 'undefined' ? true : options.enableFollowsEvents;
  helpers.preprocessKeywords(options, this);
  helpers.preprocessFollows(options, this);
...
/**
 * Returns your follow description
 * @method getFollows
 * @returns {StreamChannels.follows}
 */
StreamChannels.prototype.getFollows = function() {
  return this.follows;
};
...
var helpers = {
  ...
  /*
   * Prepares the follows to be processed later
   * @method preprocessKeywords
   * @private
   * @param {type} options
   * @param {type} streamChannels
   * @returns {StreamChannels}
   */
  preprocessFollows: function(options, streamChannels) {
    streamChannels.follows = {};

    if (options.follow instanceof Array)
    {
      for (var idx in options.follow) {
        streamChannels.follows[options.follow[idx]] = "default";
      }
    ...
    if(typeof options.follow === 'string') {
      ...
      for (var idx in options.follow.split(',')) {
        streamChannels.follows[options.follow.split(',')[idx]] = "default";
      }
    ...
    {
      var tmpFollow = [];
      for (var group in options.follow) {
        console.log(typeof options.follow[group]);
        if(! options.follow[group] instanceof Array)
        {
          throw new Error('new StreamChannels(options) - follow option invalid group parameter, should be group {} with [] of user IDs');
        }
        for (var idx in options.follow[group]) {
          if ( typeof streamChannels.follows[options.follow[group][idx]] !== 'undefined' ) {
            streamChannels.follows[options.follow[group][idx]] += ','+group;
          }
          else
          {
            streamChannels.follows[options.follow[group][idx]] = group;
          }
          if ( !( options.follow[group][idx] in tmpFollow ) ) {
              tmpFollow.push(options.follow[group][idx]);
          }
        }
      }
      options.follow = tmpFollow.join(",");
    }
    console.log(options.follow);
    console.log(streamChannels.follows);
  ...
  postprocessTweet: function(tweet,streamChannels){
    ...
    //find the follows
    if(tweet.user && tweet.user.id_str && typeof streamChannels.follows[tweet.user.id_str] !== 'undefined'){
      var follows_list = streamChannels.follows[tweet.user.id_str].split(",");
      for(var idx in follows_list)
      {
        tweet.$follows.push(follows_list[idx]);
      }
    }
  ...
  emitPosprocessedTweet: function(tweet,streamChannels){
    ...
    if(streamChannels.options.enableFollowsEvents === true){
      for(follow in tweet.$follows){
        streamChannels.emit('follows/'+tweet.$follows[follow], tweet);
      }
    }
    return streamChannels;
  },
